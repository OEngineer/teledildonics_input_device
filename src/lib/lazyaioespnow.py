...
# Modified for aioespnow usage by Ned Konz
...
class LazyAIOESPNow(aioespnow.AIOESPNow):
    ...
    async def airecv(self):
        self._check_init()
        return await super().airecv()

    @_catch_esperror
    async def arecv(self):
        self._check_init()
        return await super().arecv()

    @_catch_esperror
    async def asend(self, mac, msg=None, sync=True):
        if msg is None:
            msg, mac = mac, None  # If msg is None: swap mac and msg
        return await super().asend(mac, msg, sync)
